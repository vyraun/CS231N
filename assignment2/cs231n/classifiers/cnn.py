...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    Ha = (Hc - 2)/2 + 1
    Wa = (Wc - 2)/2 + 1
    self.params['W2'] = weight_scale*np.random.randn(num_filters*Ha*Wa, hidden_dim)
  ...
  def loss(self, X, y=None):
    ...
    # variable.                                                                #
    ############################################################################
    h, conv_cache  = conv_forward_im2col(X, W1, b1, conv_param)
    h, relu_cache1 = relu_forward(h)
    h, maxp_cache  = max_pool_forward_fast(h, pool_param)
    d1, d2, d3, d4 = h.shape
    h = h.reshape(h.shape[0], -1)
    h, affn_cache1 = affine_forward(h, W2, b2)
    h, relu_cache2 = relu_forward(h)
    h, affn_cache2 = affine_forward(h, W3, b3)
    scores = h
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    dx, dW3, db3 = affine_backward(dx, affn_cache2)
    dx = relu_backward(dx, relu_cache2)
    dx, dW2, db2 = affine_backward(dx, affn_cache1)
    dx = dx.reshape(d1, d2, d3, d4)
    dx = max_pool_backward_fast(dx, maxp_cache)
    dx = relu_backward(dx, relu_cache1)
    dx, dW1, db1 = conv_backward_im2col(dx, conv_cache)
